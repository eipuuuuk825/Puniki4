...
*
* 比留間さんの NN のクラス
* https://hbd.ist.hokudai.ac.jp/iga/ci-sample/%e3%83%8b%e3%83%a5%e3%83%bc%e3%83%
* a9%e3%83%ab%e3%83%8d%e3%83%83%e3%83%88%e3%83%af%e3%83%bc%e3%82%af/%e6%af%94%e7
* %95%99%e9%96%93%e5%90%9b-NN/
*
-----------------------------------------------*/
	...
	:n(n_), L(n.size()), eta(eta_)
{
	...
	d.resize(L);
	for (size_t l = 1; l < L; ++l) {	//d[0][i]は入力層だから誤差計算は必要ない
...
std::vector<double> so::NeuralNetwork::compute(const vector<double> &x) {
	...
	for (size_t l = 1; l < L; ++l) {	//順方向計算 第l層のk番目のニューロンの出力を計算
...
void so::NeuralNetwork::back_propagation(const vector<double> &t) {
	...
	//入力層から出力層の手前までの重みを更新
	for (size_t l = L-2; l !=0; --l) {	//第0層の誤差は計算しない
...
void so::NeuralNetwork::autoencoder(const vector<vector<double>> &x_v, int l,double epsilon ,int limit,const std::string &convergence_mode) {
	...
	while (true) {
		...
		if (convergence_mode == "deltaE") {
			if (std::abs(E - E_old) < epsilon)break;
		}
		else {
			if (E < epsilon)break;
		}
...
std::vector<double> so::NeuralNetwork::compute_lth_layer_output(const vector<double> &x, int u) {
	...
	for (int l = 1; l < u+1; ++l) {	//順方向計算 第l層のk番目のニューロンの出力を計算
		for (size_t k = 1; k < n[l] + 1; ++k) {
			double sum = 0.0;
			for (size_t i = 0; i < n[l - 1] + 1; ++i) {
				sum += z[l - 1][i] * w[l - 1][k][i];
...
double so::NeuralNetwork::error(const vector<double> &t) const{
	double sum=0.0;
	for (size_t j = 1; j < n[L - 1]+1; ++j) {
		sum += (t[j - 1] - z[L - 1][j])*(t[j - 1] - z[L - 1][j]);	//教師ベクトルの添え字は0から始まる。一方出力の0番目はバイアス項となっている
	}
	return 0.5*sum;
}

double so::NeuralNetwork::learning(
	const vector<vector<double>> &x_v,
	...
	vector<double> E_v;
	size_t size = x_v.size();		//入力ベクトルのサイズを格納
	int count = 0;					//カウント用の変数を用意
	while (true) {
		double E = 0.0;
		for (size_t p = 0; p < size; ++p) {		//p番目の入力ベクトルと入力層のニューロン数を比較し違ったらエラー
			compute(x_v[p]);		//順方向計算
			...
			if (convergence_mode == "deltaE") {		//エラーの変化率で判定する方法
				if (count != 1) {
					auto oldE_itr = E_v.end() - 2;
					double deltaE = std::abs(E - *oldE_itr);
					if (deltaE < epsilon)
						break;
				}
