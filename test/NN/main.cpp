...
int main(int argc, char **argv)
{
	...
	*
	...
	std::ifstream ifs("data/Fischer.txt");
	if (!ifs)
	...
	//データの正規化 Excelなどで直接加工たほうが楽かもしれません
	...
	for (auto &&e : table)
	{
		...
		std::copy(e.begin() + 1, e.end(), temp.begin());
	...
	for (auto &&e : table)
	{
		...
		if (species == 1.0)
			...
			throw std::runtime_error("In iris,species did not match 1.0 or 2.0,3.0");
	...
	//中間50で学習
	vector<vector<double>> x_s(x.begin() + 50, x.begin() + 100);
	vector<vector<double>> x_j(x.begin(), x.begin() + 50);
	vector<vector<double>> t_s(t.begin() + 50, t.begin() + 100);
	vector<vector<double>> t_j(t.begin(), t.begin() + 50);
	x_j.insert(x_j.end(), x.begin() + 100, x.end());
	t_j.insert(t_j.end(), t.begin() + 100, t.end());
